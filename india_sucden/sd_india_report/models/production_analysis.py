...
class ProductionAnalysis(models.Model):
    ...
    def export_data_pnl_upgrade(self):
        return self.env.ref(
                'sd_india_report.batch_report_pnl_upgrade').report_action(self)
                
    
    def export_data_pnl_bta(self):
        return self.env.ref(
                'sd_india_report.batch_report_pnl_bta').report_action(self)
            ...
            if this.picking_id:
                ...
                for kcs in this.picking_id.kcs_line.filtered(lambda r: r.product_id.id == this.product_id.id):
                    this.update({
                        ...
                        'sleeve_5_up_percent': kcs.sleeve_5_up_percent,
                        'unhulled_percent': kcs.unhulled_percent,
                        'remaining_coffee_percent': kcs.remaining_coffee_percent,
                        'blacks_percent': kcs.blacks_percent,
                        'half_monsoon_percent': kcs.half_monsoon_percent,
                        'good_beans_percent': kcs.good_beans_percent,
...
class ProductionAnalysisLineInputValue(models.Model):
    ...
    def _compute_raw_cost(self):
        for this in self:
            if this.contract_id:
                if this.contract_id.name[0:3] == 'CR' and this.exchange_rate:
