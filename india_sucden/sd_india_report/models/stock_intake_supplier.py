...
class StockIntakeQtySupplier(models.Model):
    _name = 'v.intake.qty.supplier'
    _description = 'Stock Intake Qty by Supplier'
    _auto =False

    week_name = fields.Char(string = 'Week')
    w_num = fields.Char(string = 'w_num')
    y_num = fields.Char(string = 'Year')
    supplier_id = fields.Char(string = 'Product')
    supplier_name = fields.Char(string = 'Product')
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'v_intake_qty_supplier')
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW public.v_intake_qty_supplier AS
