...
class Parser(models.AbstractModel):
    _name = 'report.report_packing_list_india'
    _inherit = ['report.report_aeroo.abstract']
    _description = 'report.report_packing_list_india'
    ...
    def get_notify(self, post_shipment):
        if post_shipment:
            shipping_id = post_shipment.shipping_id
            notify_list = [
                shipping_id.notify_1,
                shipping_id.notify_2,
                shipping_id.notify_3
            ]
            notify_combined = ', '.join(filter(None, notify_list))
            return notify_combined
