...
class SImportFile(models.Model):
    _name = "w.cancel.confirmation"
    
    def get_lot_no(self):
        lot_no = self.env['lot.stack.allocation'].browse(self._context.get('active_id'))
        return "Are you sure that you need to cancel Lot No. " + lot_no.lot_id.name + "?"
    
    message = fields.Text(string="Confirmation", readonly=True, default=get_lot_no)

    def btt_cancel(self):
        print("Button clicked")
        this = False
        this = self.env['lot.stack.allocation'].browse(self._context.get('active_id'))
        print(this.lot_id.name)
        if this.grp_id:
