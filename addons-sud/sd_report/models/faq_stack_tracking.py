        ...
        self.env.cr.execute("""
                    ...
                    SELECT row_number() OVER (ORDER BY (ss.warehouse_id, lot_list.lot_id) DESC) AS id,
                        ...
                        FROM(SELECT sm.lot_id,
                            ...
                            NULLIF(SUM(Case When sm.code NOT LIKE '%out%' then sm.mc else 0 end), 0)/NULLIF(SUM(Case When sm.code NOT LIKE '%out%' then sm.init_qty else 0 end), 0) mc_in,
                    ...
                    """)
                        ...
                        #     Case When faq_tracking.product_name = 'FAQ' Then 'FAQ' Else 'Non-FAQ' End As coffee_type, faq_tracking.* 
                        #     FROM
                        ...
                        #             Where itr_bc.picking_type_code='incoming' and ss.name not like '%HP%' and gdn.picking_type_code='transfer_out') temp Group by temp.lot_id) sum_prod) i
                        #     --Out
