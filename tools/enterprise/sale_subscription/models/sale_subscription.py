...
#        return super(SaleSubscription, self)._track_subtype(init_values)
    ...
    def create(self, vals):
        vals['code'] = (
            ...
            self.env['ir.sequence'].next_by_code('sale.subscription') or
            'New'
        )
    ...
    def name_get(self):
        ...
        for sub in self.filtered('id'):
            ...
            subscription_name = '%s' % (sub.code)
#            template_name = sub.template_id.sudo().code
            display_name = 'SUCDENCOFFEE-%s' % (subscription_name)
    ...
    def open_avis_url(self):
        if self.version == '1':
            return {
            'type': 'ir.actions.act_url',
            'url': 'https://avis.sucdencoffee.vn/web?view_type=form&model=account.payment&action=1456#view_type=form&model=account.payment&menu_id=719&action=1004',
            'target': 'self',
        }
        return {
            'type': 'ir.actions.act_url',
            'url': 'https://avis.sucdencoffee.vn/web?#id='+ self.version +'&view_type=form&model=account.payment&menu_id=719&action=1456',
            'target': 'self',
        }
