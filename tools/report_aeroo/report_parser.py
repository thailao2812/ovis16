...
class Parser(models.AbstractModel):
    ...
    def _set_lang(self, lang, obj=None, localcontext={}):
        localcontext.update(lang=lang)
        if obj is None and 'objects' in localcontext:
            obj = localcontext['objects']
    ...
    def get_stylesheet(self, report, company):
        style_io = None
...
#             'test':     self.test,
            'fields':     fields,
            # TODO xử lý in field HTML
            'convert_html_to_text': self._convert_html_to_text,
        }
        localcontext.update(ctx)
        update_localcontext = self._set_localcontext()
        if update_localcontext:
            localcontext.update(update_localcontext)
        
        self._set_lang(company.partner_id.lang, localcontext=localcontext)
        ...
        if report.styles_mode == 'default':
            ...
            style_io = BytesIO(self.get_stylesheet(report, company))
    ...
    def get_text_lst(self, text):
        """
            Remove // in str and convert str to lst
        """
        if not text:
            return
        text_lst = []
        for line in text.split('\n'):
            if line != '':
                text_lst.append(re.sub(r"\\", "", html.unescape(line)))
        return text_lst

    def _convert_html_to_text(self, field_html):
        """
            Used in file odt and loop for get values
        ...
        if field_html:
            text = text_maker.handle(field_html)
        else:
            text = ""
        return text
    ...
    def aeroo_report(self, docids, data):
        name = self._context.get('report_name')
