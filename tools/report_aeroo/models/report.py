...
from odoo.addons.report_aeroo.report_parser import AEROO_CONVERSION_COMMAND_PARAMETER
...
class IrActionsReport(models.Model):
    ...
    def render_aeroo(self, docids, data=None):
        ...
        file_data, out_code, filename = report_parser.with_context(
            context).aeroo_report(docids, data)
    ...
    charset = fields.Selection(
        '_get_encodings', string='Charset', required=True, default='utf_8')
    ...
    stylesheet_id = fields.Many2one(
        'report.stylesheets', 'Template Stylesheet', ondelete='set null')
...
class Parser(models.AbstractModel):
    ...
    process_sep = fields.Boolean(
        'Process Separately', help='Generate the report for each object separately, then merge reports.')

    in_format = fields.Selection(
        selection='_get_in_mimetypes', string='Template Mime-type')  # , default='oo-odt'
    ...
    extras = fields.Char(
        'Extra options', compute='_get_extras', size=256)
    ...
    deferred_limit = fields.Integer(
        'Deferred Records Limit', help='Records limit at which you are invited to start the deferred process.', default=80)
    ...
    wizard_id = fields.Many2one(
        'ir.actions.act_window', 'Wizard Action', ondelete='set null')
        ...
        try:
            for mod_path in odoo.addons.__path__:
                if os.path.lexists(mod_path+os.path.sep+path.split(os.path.sep)[0]):
                    ...
                    mod_name = 'odoo.addons.%s.%s' % (key, mod_name)
    ...
    def create(self, vals):
        ...
        parser = default_parser = ir_model._default_aeroo_parser(model_data)
        if res.parser_state == 'loc' and res.parser_loc:
            parser = self.load_from_file(
                res.parser_loc, res.name.lower().replace(' ', '_'))
            if not parser:
    ...
    def report_action(self, docids, data=None, config=True):
        ...
        if self.print_report_name and not len(obj) > 1:
            report_name = safe_eval(self.print_report_name, {
