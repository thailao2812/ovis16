...
class RequestPayment(models.Model):
            ...
            for npe in npe_allocation:
                ...
                for line in npe.request_payment_ids.filtered(
                    ...
                    for i in line.rate_ids.sorted(lambda x: x.date):
                        seq += 1
                        total_date = (rec.date_contract - i.date).days
                        rate = i.rate / 30 / 100
                        if seq == 1:
    ...
    def calculate_total_advance_ptbf_npe(self):
        for rec in self:
            ...
            get_all_advance_ptbf_npe_for_total = self.env['fixation.advance.ptbf.npe'].search([
                ('name', 'in', ['Total', 'Còn lại/ Remain Payment:']),
                ('request_payment_id', '=', rec.id),
            ])
        ...
        for rec in self:
            ...
            if rec.type == 'ptbf':
                ...
                if other_request_payment_ptbf:
                    if rec.type_of_ptbf_payment not in ['fixation', 'advance']:
                        payment_qty = 0
                    else:
                        payment_qty = rec.payment_quantity
                    rec.balance = purchase_contract.total_qty - sum(other_request_payment_ptbf.mapped('payment_quantity')) - payment_qty
                else:
