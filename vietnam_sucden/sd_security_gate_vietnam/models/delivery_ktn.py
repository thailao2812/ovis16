...
class DeliveryKTN(models.Model):
    _name = 'delivery.ktn'
    ...
    def compute_paid_qty(self):
        for rec in self:
            ...
            if self.env.context.get('seq'):
                delivery_ready_in_current_contract = self.env['delivery.ktn'].search([
                ...
                delivery_ready_other_contract = self.env['delivery.ktn'].search([
                    ('delivery_id', '=', rec.delivery_id.id),
                    ('request_payment_id.purchase_contract_id', '!=', self.env.context.get('purchase_contract_id'))
                ])
