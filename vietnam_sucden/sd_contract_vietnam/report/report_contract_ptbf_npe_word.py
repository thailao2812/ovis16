...
class Parser(models.AbstractModel):
    _name = 'report.report_ptbf_npe_word'
    _inherit = ['report.report_aeroo.abstract']
    _description = 'report.report_ptbf_npe_word'
    ...
    def get_rool_text(self, roll_id):
        ...
        if roll_id:
            ...
            if roll_id.contract_id.rolling_ids.sorted(key=lambda l: l.date, reverse=True).filtered(
                    lambda x: x.date < roll_id.date):
                roll_before_id = roll_id.contract_id.rolling_ids.sorted(key=lambda l: l.date, reverse=True).filtered(
                    lambda x: x.date <= roll_id.date)[0]
                ...
                if roll_before_id.diff_price < 0:
                    ...
                    vn += u'$/tấn (' + u'%s-%s' % (
                    _(self.convert_month(roll_before_id.date_fixed)) + _(roll_before_id.date_fixed)[2:4],
                    _(abs(int(roll_before_id.diff_price))))
                    vn += u'$/Mt)'
                ...
                if roll_before_id.diff_price < 0:
                    vn += u' trừ lùi ' + u'%s' % (_(abs(int(roll_before_id.diff_price))))
                    vn += u'$/tấn (' + u'%s-%s' % (
                    _(self.convert_month(roll_id.contract_id.date_fix)) + _(roll_id.contract_id.date_fix)[2:4],
                    _(abs(int(roll_before_id.diff_price))))
                    vn += u'$/Mt)'
                    en += u' in London LIFFE minus (-) %s' % (_(abs(roll_before_id.diff_price)))
                    en += u'$/Mt'
            ...
            if roll_id.diff_price < 0:
                ...
                vn1 += u'$/tấn (' + u'%s-%s' % (
                _(self.convert_month(roll_id.date_fixed)) + _(roll_id.date_fixed)[2:4], _(abs(int(roll_id.diff_price))))
                vn1 += u'$/Mt)'
    ...
    def get_myear(self, date_fix):
        ...
        date = self.env['res.users']._convert_user_datetime(
            fields.Datetime.to_string(date_fix))

        if date.strftime('%m') in ('01', '02'):
            return 'F%s' % str(date.strftime('%y'))
        if date.strftime('%m') in ('03', '04'):
            return 'H%s' % str(date.strftime('%y'))
        if date.strftime('%m') in ('05', '06'):
            return 'K%s' % str(date.strftime('%y'))
        if date.strftime('%m') in ('07', '08'):
            return 'N%s' % str(date.strftime('%y'))
        if date.strftime('%m') in ('09', '10'):
            return 'U%s' % str(date.strftime('%y'))
        if date.strftime('%m') in ('11', '12'):
            return 'X%s' % str(date.strftime('%y'))
        return 1
